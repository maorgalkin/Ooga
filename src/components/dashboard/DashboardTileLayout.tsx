...
// lg+   → 2-column: [categories → pie] LEFT  |  [chips 2-col grid] RIGHT
//
// Desktop fuse behaviour (category with transactions clicked):
//   • Categories panel slides out (height → 0), pie expands to full width of LEFT column.
//   • Block-level X + click-away closes the transaction view and re-detaches categories.
//   • 0-transaction category → no fuse, tile rendered as non-interactive.
...
export const DashboardTileLayout: React.FC<DashboardTileLayoutProps> = ({
  ...
  // ref wrapping the ExpenseChart for click-away
  const expensesCardRef = useRef<HTMLDivElement>(null);

  // ── Click-away: desktop only, when fused ──────────────────────────────────
  useEffect(() => {
    if (!isFused) return;
    ...
    // Defer so the click that triggered fuse doesn't immediately close it
    const id = setTimeout(() => {
    ...
    () =>
      transactions
        .filter((t) => {
          ...
          return d.getMonth() === monthDate.getMonth() && d.getFullYear() === year && t.type === 'income';
        })
  ...
  const usagePct =
    analysis.totalBudgeted > 0 ? Math.min(100, (analysis.totalSpent / analysis.totalBudgeted) * 100) : 0;
  ...
  // ── Chip data ──────────────────────────────────────────────────────────────
  type Chip = { label: string; value: string; sub?: string; color: string; bg: string; Icon: React.ElementType };

  const chips: Chip[] = [
    { label: 'Income',    value: formatCurrency(monthIncome),           color: 'text-green-700 dark:text-green-400',  bg: 'bg-green-50 dark:bg-green-900/20',   Icon: TrendingUp   },
    { label: 'Expenses',  value: formatCurrency(analysis.totalSpent),   color: 'text-red-600 dark:text-red-400',      bg: 'bg-red-50 dark:bg-red-900/20',       Icon: TrendingDown },
    {
  ...
  if (overBudgetCount > 0)
    chips.push({ label: 'Over budget', value: String(overBudgetCount), sub: `categor${overBudgetCount === 1 ? 'y' : 'ies'}`, color: 'text-red-600 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-900/20', Icon: AlertTriangle });
  if (monthlyBudget?.adjustment_count && monthlyBudget.adjustment_count > 0)
    chips.push({ label: 'Adjustments', value: String(monthlyBudget.adjustment_count), sub: 'next month', color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-900/20', Icon: Calendar });
  ...
  // ── Sub-renderers ──────────────────────────────────────────────────────────
  const renderChipCard = ({ label, value, sub, color, bg, Icon }: Chip) => (
    <div key={label} className={`rounded-xl p-3 ${bg} border border-white/60 dark:border-gray-700/50`}>
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
      {sortedCategories.length === 0 ? (
        <div className="p-6 text-center text-gray-400 text-sm">
          ...
          <button onClick={() => navigate('/?tab=budget')} className="mt-2 text-purple-600 dark:text-purple-400 underline text-xs">
            Set up your budget →
          </button>
        </div>
      ) : (
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {/* ── LARGE: [categories → pie] LEFT  |  [chips 2-col] RIGHT ── */}
      <div className="hidden lg:flex gap-5">

        {/* LEFT column */}
        <div className="flex-1 min-w-0 space-y-4">

          {/* Block-level X — shown when fused, above the expenses card */}
          <AnimatePresence>
            {isFused && (
              <motion.div
          ...
          {/* Categories — animate out (height collapses) when fused */}
          <AnimatePresence>
            {!isFused && (
              <motion.div
                key="cat-grid"
          ...
          {/* ExpenseChart — click-away ref wraps only this */}
        ...
        {/* RIGHT column: chips in symmetrical 2-col grid */}
